...
class BALDMDPSelectionPolicy(DataSelectionPolicy):
    def select_data(self, dataset, preference_model, batch_size):
        ...
        data = dataset.samples
        #_, pref_probs, _, _, pref_probs_samples = preference_model.generate_preferences(data)

        sum_of_variances_1 = preference_model.generate_sum_of_variances(data[0])
        sum_of_variances_2 = preference_model.generate_sum_of_variances(data[1])
        
        #bald_scores = self._entropy(torch.mean(pref_probs_samples, axis=1)) - torch.mean(self._entropy(pref_probs_samples))
        #var_scores = pref_probs_samples.var(axis=1)
        var_scores = sum_of_variances_1 + sum_of_variances_2
