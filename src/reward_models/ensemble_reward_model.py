...
class EnsembleRewardModel(UncertaintyAwareRewardModel):
    ...
    def train(self, reward_tuples, full_retrain=True):
        """ Input has shapes ([N,3], [N])"""
        if full_retrain:
            del self.models
            self.models = []
            for i in range(self.args['ensemble_size']):
        ...
        self._add_data(reward_tuples)

        for i in range(self.args['ensemble_size']): 

            full_dataset = torch.utils.data.TensorDataset(self.full_data[0], self.full_data[1])
    ...
    def _add_data(self, reward_tuples):
        #rows = torch.stack(preference_tuples, dim=1)
        if self.full_data is None:
            self.full_data = reward_tuples
        else: 
            self.full_data = (
                torch.cat((self.full_data[0], reward_tuples[0]), dim=0),
                torch.cat((self.full_data[1], reward_tuples[1]), dim=0)
            )
    ...
    def _compute_loss(
        ...
        trajectories, gt_rewards = inputs[0], inputs[1]
        predicted_rewards = self.compute_trajectory_rewards_from_model(trajectories, model_index, use_standardizer=False)

        loss = self.loss_fn(predicted_rewards, gt_rewards.reshape(gt_rewards.shape[0], 1))
        return loss
