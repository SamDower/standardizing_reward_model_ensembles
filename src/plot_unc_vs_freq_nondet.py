...
def load_trajectories(exp_name, seed, epoch):
    ...
    traj_1 = traj_1.reshape((traj_1.shape[0] // 48, 16, 3))
    traj_2 = traj_2.reshape((traj_2.shape[0] // 48, 16, 3))
    return traj_1, traj_2

def calculate_sas_frequencies(traj_1, traj_2):
    sa_frequencies = np.zeros((36, 4, 36))
    for i in range(traj_1.shape[0]):
        for t in range(traj_1.shape[1]):
            sa_frequencies[traj_1[i][t][0]][traj_1[i][t][1]][traj_1[i][t][2]] += 1
            sa_frequencies[traj_2[i][t][0]][traj_2[i][t][1]][traj_2[i][t][2]] += 1
...
def generate_reward_variance_tables(exp_name, seed, epoch):
    ...
    markov_decision_process = LavaPathNonDetMDP(config['mdp'], device)
    #reward_standardizer = StarcNonDetRewardStandardizer(config['reward_standardizer'], device, markov_decision_process)
    reward_standardizer = StarcNonDetSimplerRewardStandardizer(config['reward_standardizer'], device, markov_decision_process)
    pref_model = EnsembleNonDetPreferenceModel(device, reward_standardizer, **config['pref_model'])
    ...
    return pref_model.raw_reward_variances_table, pref_model.stand_reward_variances_table
...
if __name__ == "__main__":
    ...
    exp_name = "36nd_15_varp_none"
    seed = 6480#8567#1878
    epoch = 49

    # exp_name = "36nd_15_varprel_starc"
    # seed = 6904
    # epoch = 20

    traj_1, traj_2 = load_trajectories(exp_name, seed, epoch)

    sas_frequencies = calculate_sas_frequencies(traj_1, traj_2)
    ...
    for s in range(36):
        for a in range(4):
            mask = raw_reward_variances_table[s,a,:].detach().cpu().numpy() > 0
            freq = sas_frequencies[s,a,:][mask]
            unc = raw_reward_variances_table[s,a,:].detach().cpu().numpy()[mask]
            axes[0].scatter(freq, unc, color='red', alpha=1)
    #axes[0].scatter(sas_frequencies.flatten(), raw_reward_variances_table.detach().cpu().numpy().flatten(), color='red', alpha=1)
    axes[0].set_xlabel('Frequency')
    axes[0].set_ylabel('Uncertainty')
    axes[0].set_title('Frequency vs Uncertainty (No Standardizing)')

    #axes[1].scatter(sas_frequencies.flatten(), standardized_reward_variances_table[:,:,:,0].detach().cpu().numpy().flatten(), color='red', alpha=1)
    ...
    for s in range(36):
        for a in range(4):
            col = 'red'
            zorder = 1
            #if np.random.random(1)<0.05 and color_index < len(colors):
            if sas_frequencies[s,a,:].max() > 400 and color_index < len(colors):
                col = colors[color_index]
                color_index += 1
                print(f"set color {col}")
                zorder = 2
            mask = standardized_reward_variances_table[s,a,:,0].detach().cpu().numpy() > 0
            freq = sas_frequencies[s,a,:][mask]
            unc = standardized_reward_variances_table[s,a,:,0].detach().cpu().numpy()[mask]
            axes[1].scatter(freq, unc, color=col, alpha=1, zorder=zorder)
    

    axes[1].set_xlabel('Frequency')
    axes[1].set_ylabel('Uncertainty')
    axes[1].set_title('Frequency vs Uncertainty (With Standardizing)')
    plt.savefig("freq_vs_unc_nondet", dpi=300)  # Adjust dpi as needed
    plt.close()
