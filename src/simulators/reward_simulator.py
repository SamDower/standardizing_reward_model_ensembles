...
from reward_models import UncertaintyAwareRewardModel
...
class RewardOptimizationSimulator:
    def __init__(self,
                 ...
                 uncertainty_aware_reward_model: UncertaintyAwareRewardModel,
                 trainer,#: Trainer,
                 exp_name,
                 seed):
        """
        ...
        self.uncertainty_aware_reward_model = uncertainty_aware_reward_model
    ...
    def run(self, args, debug=True):
        ...
        for epoch in range(args['epochs']):
            ...
            trajectories = self.data_selection_policy.select_data(self.pool_dataset, self.uncertainty_aware_reward_model, batch_size=args['batch_size'])
            # 2. Generate Ground Truth Rewards from Oracle
            rewards = self.markov_decision_process._generate_gt_trajectory_reward(trajectories)
            reward_tuples = (trajectories, rewards)
            # 3. Train/Eval the uncertainty-aware preference model
            self.uncertainty_aware_reward_model.train(reward_tuples, full_retrain=args['full_retrain'])

            print("Generating raw table")
            self.uncertainty_aware_reward_model.generate_raw_reward_table()

            # Calculate STARC requirements
            print("Caluculating values for standardizer")
            self.reward_standardizer.calculate_values(self.uncertainty_aware_reward_model)
            print("Calculating norms for standardizer")
            self.reward_standardizer.calculate_norms(self.uncertainty_aware_reward_model)

            print("Generating stand table")
            self.uncertainty_aware_reward_model.generate_stand_reward_table()
            ...
            if (epoch+1) % args['evaluate_every'] == 0:
                ...
                for i in range(len(self.uncertainty_aware_reward_model.models)):
                    torch.save(self.uncertainty_aware_reward_model.models[i].state_dict(), 
                ...
                rw_eval_dict = self.uncertainty_aware_reward_model.eval(self.test.samples, self.markov_decision_process)
...
def save_latest_full_data(eval_rewards, exp_name, config, seed):
    ...
    trajectories = latest_eval_dict['full_data']
    utils.tensors_to_csv([trajectories.flatten()], ['Trajectories'], f"experiments/{exp_name}/{seed}/raw/full_data_{epoch}")
