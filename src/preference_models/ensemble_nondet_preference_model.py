...
class EnsembleNonDetPreferenceModel(UncertaintyAwarePreferenceModel):
    def __init__(self, device, reward_standardizer, **kwargs):
        ...
        self._data_min = None
        self._data_max = None
        self.raw_reward_table = torch.zeros(self.args['ensemble_size'], 36, 4, 36).to(self.device)
        self.raw_reward_variances_table = torch.zeros(36, 4, 36).to(self.device)
        self.stand_reward_table = torch.zeros(self.args['ensemble_size'], 36, 4, 36, 16).to(self.device)
        self.stand_reward_variances_table = torch.zeros(36, 4, 36, 16).to(self.device)
    ...
    def generate_raw_reward_table(self):
        for s in range(36):
            for a in range(4):
                for i in range(4):
                    next_s = self.reward_standardizer.markov_decision_process.possible_next_states_matrix[s][a][i]
                    sas = torch.tensor([s, a, next_s]).to(self.device).reshape(1, 3)
                    _, all_raw_rewards = self.generate_state_actions_reward(sas, -1, use_standardizer=False)
                    self.raw_reward_table[:,s,a,next_s] = all_raw_rewards
                    self.raw_reward_variances_table[s][a][next_s] = all_raw_rewards.var()
        ...
        for s in range(36):
            for a in range(4):
                for i in range(4):
                    ...
                    sas = torch.tensor([s, a, next_s]).to(self.device).reshape(1, 3)
                    for t in range(16):
                        _, all_stand_rewards = self.generate_state_actions_reward(sas, t, use_standardizer=True)
                        self.stand_reward_table[:,s,a,next_s,t] = all_stand_rewards
                        self.stand_reward_variances_table[s][a][next_s][t] = all_stand_rewards.var()
    ...
    def generate_reward(self, trajectories, use_standardizer=True, use_table=False):
        ...
        if not use_table:
            ...
            for t in range(trajectories.shape[1]):
                ...
                next_state_indices = next_states.view(1, N).expand(E, N)              # shape [E, N]
                if use_standardizer:
                    rewards += self.stand_reward_table[ensemble_indices, state_indices, action_indices, next_state_indices, t].T # shape [N, E]
                else:
                    rewards += self.raw_reward_table[ensemble_indices, state_indices, action_indices, next_state_indices].T # shape [N, E]
    ...
    def compute_state_action_rewards_from_model(self, state_actions, t_index, model_index, use_standardizer=True):
        ...
        if use_standardizer:
            standardized_rewards = self.reward_standardizer.standardize(
                state_actions, t_index, self.raw_reward_table[model_index], model_index, nondet=True
            )
            return standardized_rewards.reshape(standardized_rewards.shape[0], 1)
        else:
            one_hot_inputs = torch.cat(
            (F.one_hot(state_actions[:,0], num_classes=36), 
             F.one_hot(state_actions[:,1], num_classes=4),
             F.one_hot(state_actions[:,2], num_classes=36)
            ), dim = 1).float()
